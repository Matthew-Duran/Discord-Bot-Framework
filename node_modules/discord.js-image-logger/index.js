...
module.exports = (client, options) => {
  const DIL = {
    ...
    serverWide: (options && typeof options.serverWide !== 'undefined' ? options && options.serverWide : true),
  ...
  client.on("ready", () => {
    ...
    client.user.setActivity(`over all attachments` , { type: "WATCHING" });
  });
  ...
  client.on("message", (msg) => {
    ...
    if (DIL.method === "link") {
      ...
      embed.setTitle(msg.author.username + '  ' + "Uploaded an attachment")
      ...
      attachments.forEach(at => {
        embed.addField('Filename', at.filename, true);
     embed.addField("User ID", msg.author.id +"  " + '|' + "  " + `[Link](${at.url})`, true )
      

      });
      embed.setColor("0xffa9e8");
      logchan.send({
        embed
      }).catch(console.log);
    };
