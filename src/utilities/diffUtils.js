...
const {
  Collection, GuildMember, Message, Emoji, BitField,
} = require('discord.js');
...
const getChanges = (oldInstance, newInstance, key, changes = {}) => {
  if (Array.isArray(oldInstance[key]) && Array.isArray(newInstance[key]) && !areEqualArrays(oldInstance[key], newInstance[key])) {
    ...
    if (isBitField(oldInstance[key]) && isBitField(newInstance[key]) && (oldInstance[key].bitfield === newInstance[key].bitfield)) return;
    changes[key] = {
      old: oldInstance[key],
      new: newInstance[key],
    };
...
const diff = (oldInstance, newInstance) => {
  ...
  } else if ([oldInstance, newInstance].every((i) => i instanceof Emoji)) Object.keys(flatten(newInstance, { roles: true })).forEach((key) => getChanges(oldInstance, newInstance, key, changes));
