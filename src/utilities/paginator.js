...
module.exports = class Paginator {
  constructor({ channel, user }, fields, options) {
    ...
    if (options.numberFields) this.fields = this.fields.map((field, index) => ({ ...field, name: `${index + 1}. ${field.name}` }));
    if (options.splitLongFields) this.fields = this.splitLongFields(this.fields);
    if (typeof options.chunkSize !== 'number' || options.chunkSize < 1 || options.chunkSize > 12) options.chunkSize = 5;
    if (typeof options.defaultPage !== 'number' || !(options.defaultPage >= 0 && options.defaultPage <= fields.length)) options.defaultPage = 1;
  ...
  splitText(text, {
    ...
    if (!Array.isArray(characters)) if (characters instanceof String) characters = [characters];
    for (const char of characters) {
      try {
        const result = splitMessage(text, {
          maxLength, char, prepend, append,
        });
        return result;
      } catch (err) {
  ...
  splitLongFields(fields, lengths = { name: 256, value: 1024 }) {
    ...
    newFields = newFields.map((field) => {
      ...
      if (!field.nameEdited && field.name.length > lengths.name) newField.name = `${field.name.substring(0, lengths.name - 3)}...`;
      return newField;
    });
    return newFields;
  }
