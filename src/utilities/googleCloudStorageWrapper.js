...
module.exports = class StorageWrapper {
  ...
  async uploadAttachment(attachment) {
    ...
    return new Promise((resolve, reject) => got.stream(attachment.url)
  ...
  async uploadAttachments(...attachments) {
    if (attachments.length > 1 && attachments.every((a) => a instanceof MessageAttachment)) return Promise.all(attachments.map((a) => this.uploadAttachment(a)));
    if (attachments[0] instanceof Array || attachments[0] instanceof Collection) return Promise.all(attachments[0].map((a) => this.uploadAttachment(a)));
  }
  ...
  async listFiles(prefix) {
    ...
    return Promise.all(files.map(async (file) => {
      ...
      if (file.name.match(/[0-9]{15,25}_[0-9]{15,25}_.*/gi)) {
        ...
        return {
          userId, messageId, name: rest.join('_'), url,
        };
      }
      return { name: file.name, url };
    }));
  ...
  async deleteFiles(name, ignoreIds = true) {
    const [files] = await this.bucket.getFiles();
    if (!name) return Promise.all(files.map((f) => f.delete()));
    if (name) {
