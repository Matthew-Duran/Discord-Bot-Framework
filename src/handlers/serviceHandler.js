...
class ServiceHandler {
  async initialize(client) {
    ...
    this.registerUsedEvents()
      ...
      .registerTimedEvents();
    if (!this.client.serviceHandler) this.client.serviceHandler = this;
  ...
  enableService(id) {
    ...
    if (this.checkIfValid(service)) service.enable();
  }

  disableService(id) {
    const service = this.services[id];
    if (this.checkIfValid(service)) service.disable();
  }
  ...
  async runClientEvent(event, args) {
    if (!this.usedEvents.has(onText(event))) return;
    ...
    return Promise.all(Object.values(this.services)
      ...
      .map((service) => {
        ...
        if (service.fetchPartials) return context.fetchPartials().then((ctx) => service[onText(event)](ctx));
  ...
  registerClientEvents() {
    ...
    for (const event of this.usedClientEvents) this.client.on(event, (...args) => this.runClientEvent(event, args));
    return this;
  }

  registerTimedEvents() {
    this.usedTimedEvents = new Set([...this.timedEvents].filter((interval) => this.usedEvents.has(everyText(interval))));
    for (const event of this.usedTimedEvents) setInterval(() => this.runTimedEvent(event, [event]), this.intervals[event] * 1000);
    return this;
  }
}
