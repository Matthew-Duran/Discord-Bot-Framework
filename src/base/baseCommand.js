...
module.exports = class BaseCommand extends Command {
  constructor(client, commandInfo) {
    ...
    if (this.argsCollector) this.argsCollector.promptLimit = 0;
  ...
  checkInhibitor(commandInfo) {
    if (commandInfo.inhibitor && typeof commandInfo.inhibitor === 'function') {
      this.client.dispatcher.addInhibitor((commandMessage) => {
        if (commandMessage.command.name === commandInfo.command.name && commandMessage.command.memberName === commandInfo.command.memberName) return commandInfo.inhibitor(commandMessage);
        return false;
      });
    }
  }
  ...
  onCancel(ctx) {
    ...
    if (!ctx.collectedArgs.values || (Array.isArray(ctx.collectedArgs.values) && !ctx.collectedArgs.values.length)) return ctx.embed({ description: 'No arguments specified' });
    if (ctx.collectedArgs.values && ctx.collectedArgs.values.length && ctx.collectedArgs.cancelledArg) {
      const { cancelledArg, cancelledValue } = ctx.collectedArgs;
      let type; let
        isOneOf = false;
      if (cancelledArg.type.id.includes('|')) type = cancelledArg.type.id.split('|').map((id) => id.split('-').join(' ')).map((t) => `\`${toTitleCase(t)}\``);
      ...
      if (!type) type = cancelledArg.type.id.split('-').join(' ');
