...
module.exports = class Context {
  ...
  async fetchPartials() {
    ...
    this.resolvedPartials = await Promise.all(props.map((prop) => {
      if (this[prop] && this[prop].partial) return this[prop].fetch().catch((...args) => this.partialErrorHandler(...args));
  ...
  async fetchAuditLog() {
    ...
    if (!hasAuditLogPermission) return Promise.reject('No audit log permission.');

    let types = [];
    if (Object.keys(AuditLogEvents).includes(this.event)) types = [this.event];
    else if (this.event === 'channelPinsUpdate') types = ['messagePin', 'messageUnpin'];
    else if (this.event === 'guildMemberAdd' && this.member.user.bot) types = ['botAdd'];
    else if (this.event === 'guildMemberAdd' && !this.member.user.bot) types = ['inviteUpdate'];
    else if (this.event === 'guildMemberRemove') types = ['memberKick', 'memberPrune'];
    if (this.event === 'guildMemberUpdate') types.push('memberRoleUpdate');
    if (this.event === 'webhookUpdate') types.push('webhookCreate', 'webhookDelete');
    types = [...new Set(types)];
    const entries = [];
  ...
  get changes() {
    ...
    if (this.isUpdateEvent && this._changes) return Object.keys(this._changes).length ? this._changes : null;
    if (this.event === 'guildUpdate') this._changes = diff(this.oldGuild, this.newGuild);
    else if (this.event === 'channelUpdate') this._changes = diff(this.oldChannel, this.newChannel);
    else if (this.event === 'emojiUpdate') this._changes = diff(this.oldEmoji, this.newEmoji);
    else if (['guildMemberUpdate', 'voiceStateUpdate', 'presenceUpdate'].includes(this.event)) this._changes = diff(this.oldMember, this.newMember);
    else if (this.event === 'messageUpdate') this._changes = diff(this.oldMessage, this.newMessage);
    else if (this.event === 'roleUpdate') this._changes = diff(this.oldRole, this.newRole);
  ...
  embed(...data) {
    if (typeof data[0] === 'string') return this.message ? this.message.embed({ description: data[0] }) : this.channel.send(new MessageEmbed({ description: data[0] }));
  ...
  get nadekoConnector() {
    if (this._nadekoConnector instanceof NadekoConnectorClient) return this._nadekoConnector;

    if (!this.guildStorage) return undefined;
    const nadekoConnector = this.guildStorage.get('nadekoconnector');
    if (!nadekoConnector) return undefined;
