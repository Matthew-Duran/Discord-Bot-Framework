...
module.exports = {
  ...
  extender: (Message) => class CommandoContextMessage extends Message {
    ...
    async run() {
      ...
      if (!this.member && !messageContext.member) await this.guild.members.fetch(messageContext.author);
    ...
    checkGuildCommandPermissions(ctx, commandName) {
      ...
      if (ctx.guildStorage.has('commandPermissions')) commandPermissions = ctx.guildStorage.get('commandPermissions');
    ...
    checkGlobalCommandPermissions(ctx, commandName) {
      let commandPermissions = [];
      if (ctx.globalStorage.has('commandPermissions')) commandPermissions = ctx.globalStorage.get('commandPermissions');
      else ctx.globalStorage.set('commandPermissions', []);
      return this.checkCommandPermissions(ctx, commandName, commandPermissions);
    }

    checkCommandPermissions(ctx, commandName, commandPermissions) {
      let permitted = true; let
        blockReason = {};
